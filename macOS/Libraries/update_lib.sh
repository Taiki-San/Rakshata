...
do
	...
	then
		FINAL_DL_URL="$(grep "<title>Release " $DL_FILE)"
		if [ "$FINAL_DL_URL" = '' ]
		then
		...
		FINAL_DL_URL="https://curl.haxx.se/download/curl-$NEW_VERSION.tar.gz"
	fi
	...
	then
		...
		#Compiling
		clang -O3 -c -DHAVE_USLEEP=1 -DSQLITE_ENABLE_FTS3 -DSQLITE_ENABLE_FTS3_PARENTHESIS -DSQLITE_ENABLE_ICU -I../../include/ $SQLITE_SOURCE_FILE -o sqlite3.o
...
#		patch lib/vtls/openssl.c < ../../7.48.patch
