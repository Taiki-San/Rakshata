...
static unzFile unzOpenInternal (const void *path, zlib_filefunc64_32_def* pzlib_filefunc64_32_def, int is64bitOpenFunction)
{
	...
	uint64_t number_entry_CD;      /* total number of entries in the central dir (same than number_entry on nospan) */
	...
	else
	{
		...
		/* number of this disk */
		if(unz64local_getShort(&us.z_filefunc, us.filestream, (uint16_t *) &number_disk)!=UNZ_OK)
			err = UNZ_ERRNO;

		/* number of the disk with the start of the central directory */
		if(unz64local_getShort(&us.z_filefunc, us.filestream, (uint16_t *) &number_disk_with_CD)!=UNZ_OK)
			err = UNZ_ERRNO;

		/* total number of entries in the central dir on this disk */
		if(unz64local_getShort(&us.z_filefunc, us.filestream, (uint16_t *) &uL)!=UNZ_OK)
			err = UNZ_ERRNO;
...
static int unz64local_CheckCurrentFileCoherencyHeader (unz64_s* s, uint32_t* piSizeVar, uint64_t * poffset_local_extrafield, uint32_t  * psize_local_extrafield)
{
	...
	uint64_t size_filename, size_extra_field;
	int err = UNZ_OK;
	...
	*psize_local_extrafield = 0;
	...
	if(unz64local_getShort(&s->z_filefunc, s->filestream, (uint16_t *) &uData) != UNZ_OK)
		err = UNZ_ERRNO;

	if(unz64local_getShort(&s->z_filefunc, s->filestream, (uint16_t *) &uFlags) != UNZ_OK)
		err = UNZ_ERRNO;
	...
	if(unz64local_getShort(&s->z_filefunc, s->filestream, (uint16_t *) &size_filename) != UNZ_OK)
		err = UNZ_ERRNO;
	...
	*piSizeVar += (uint32_t)size_filename;
	...
	if(unz64local_getShort(&s->z_filefunc, s->filestream, (uint16_t *) &size_extra_field) != UNZ_OK)
		err = UNZ_ERRNO;
 ...
 */
extern int  unzOpenCurrentFile3 (unzFile file, int* method, int* level, int raw)
{
	int err = UNZ_OK;
	uint32_t iSizeVar;
	unz64_s* s;
...
extern int  unzReadCurrentFile  (unzFile file, voidp buf, unsigned len)
{
	...
	if((len>pfile_in_zip_read_info->rest_read_compressed+
	   ...
	   (pfile_in_zip_read_info->raw))
		pfile_in_zip_read_info->stream.avail_out =
		(uint32_t)pfile_in_zip_read_info->rest_read_compressed+
		pfile_in_zip_read_info->stream.avail_in;
	...
	while (pfile_in_zip_read_info->stream.avail_out>0)
	{
		...
		else if(pfile_in_zip_read_info->compression_method==Z_BZIP2ED)
		{
			...
			pfile_in_zip_read_info->_crc32 = _crc32(pfile_in_zip_read_info->_crc32,bufBefore, (uint32_t)(uOutThis));
...
extern uint64_t  unzGetOffset (unzFile file)
{
	uint64_t offset64;
